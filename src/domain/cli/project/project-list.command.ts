...
export default class ProjectListCommand implements CommandType {
  constructor(
  ...
  async handler(_args: string[]): Promise<void> {
    ...
    for await (const projectWorkspace of this.workspace.listProjects()) {
      ...
      console.log(
        YAML.stringify({
          Selected: project.id === configuration.project_selected
            ? true
            : undefined,
          ID: project.id,
          Title: project.title,
          CreatedAt: project.cratedAt?.toLocaleString(undefined, {
            dateStyle: "full",
            timeStyle: "full",
          }),
          UpdatedAt: project.updatedAt?.toLocaleString(undefined, {
            dateStyle: "full",
            timeStyle: "full",
          }),
        }),
